Dailies = {
    name = "DailiesChecker"
}
-- colors for the text
Dailies.colors = {
    main = "3189f3",
    args = "f4dd48",
    done_state = "82b893",
    completed_state = "31d9f3",
    missing_state = "d56549",
    partial_state = "f3e131",
    accepted_state = "a375f4",
    category_header = "f3e131",
    alert = "41eb02"
}
-- dailies quest db
Dailies.db = {
    tbcdungeons = {
        name = "TBC Normal Dungeon",
        quests = { 11389, 11371, 11376, 11383, 11364, 11500, 11385, 11387, 11500 },
        max = 1,
        subtype = "tbcdungeons"
    },
    tbcheroics = {
        name = "TBC Heroic Dungeon",
        quests = { 11369, 11384, 11382, 11363, 11362, 11375, 11354, 11386, 11373, 11378, 11374, 11372, 11368, 11388, 11370, 11499 },
        max = 1,
        subtype = "tbcdungeons"
    },
    skettis = {
        name = "Skettis",
        quests = { 11008, 11085 },
        max = -1,
        subtype = "skettis"
    },
    ogrila = {
        name = "Ogri'la",
        quests = { 11080, 11023, 11066, 11051 },
        max = -1,
        subtype = "ogrila"
    },
    netherwing = {
        name = "Netherwing",
        quests = { 11015, 11018, 11017, 11016, 11020, 11035, 11076, 11077, 11055, 11086, 11101, 11097 },
        max = -1,
        subtype = "netherwing"
    },
    sso = {
        name = "Shattered Sun Offensive",
        quests = {
            11875, 11523, 11536, 11525, 11547, 11533, 11537, 11544, 11877, 11514, 11515, 11516, 11880,
            11542, 11539, 11521, 11541, 11546, 11540, 11543
        },
        max = -1,
        subtype = "sso"
    },
    pvpbg = {
        name = "PvP Battlegrounds",
        quests = { 11340, 11337, 11336, 11335, 11342, 11338, 11339, 11341 },
        max = 1,
        subtype = "pvp"
    },
    pvpzone = {
        name = "PvP Zones",
        quests = { 11502, 11503, 11505, 11506, 10106, 10110 },
        max = -1,
        subtype = "pvp"
    },
    tbccooking = {
        name = "TBC Cooking",
        quests = { 11379, 11381, 11377, 11380 },
        max = 1,
        subtype = "tbcprofessions"
    },
    tbcfishing = {
        name = "TBC Fishing",
        quests = { 11665, 11667, 11669, 11668, 11666 },
        max = 1,
        subtype = "tbcprofessions"
    },
    wotlkdungeons = {
        name = "WOTLK Normal Dungeon",
        quests = { 13240, 13243, 13244, 13241 },
        max = 1,
        subtype = "wotlkdungeons"
    },
    wotlkheroics = {
        name = "WOTLK Heroic Dungeon",
        quests = { 13254, 13256, 13250, 13245, 13246, 13248, 13247, 13253, 13252, 13249 },
        max = 1,
        subtype = "wotlkdungeons"
    },
    hodir = {
        name = "Sons of Hodir",
        quests = { 12981, 12977, 13006, 12994, 13003, 13559 },
        max = -1,
        subtype = "hodir"
    },
    ebon = {
        name = "Knights of the Ebon Blade",
        quests = { 12995, 13069, 13071, 12813, 12815, 12838 },
        max = -1,
        subtype = "ebon"
    },
    oracles = {
        name = "The Oracles",
        quests = { 12704, 12735, 12737, 12736, 12726, 12761, 12762 },
        max = -1,
        subtype = "oracles"
    },
    frenzyheart = {
        name = "Frenzyheart",
        quests = { 12702, 12758, 12734, 12741, 12732, 12703, 12760, 12759 },
        max = -1,
        subtype = "frenzyheart"
    },
    warsong = {
        name = "Warsong Offensive",
        quests = { 12170, 12284, 12280, 12288, 12270, 12317, 12324, 12315, 12432 },
        max = -1,
        subtype = "warsong"
    },
    kaluak = {
        name = "Kalu'ak",
        quests = { 11960, 11945, 11472 },
        max = -1,
        subtype = "kaluak"
    },
    wyrmrest = {
        name = "Wyrmrest",
        quests = { 11940, 13414, 12372 },
        max = -1,
        subtype = "wyrmrest"
    },
    wotlkcooking = {
        name = "WOTLK Cooking",
        quests = { 13115, 13113, 13112, 13116, 13114 },
        max = 1,
        subtype = "wotlkprofessions"
    },
    wotlkfishing = {
        name = "WOTLK Fishing",
        quests = { 13833, 13834, 13836, 13832, 13830 },
        max = 1,
        subtype = "wotlkprofessions"
    },
    wotlkjc = {
        name = "WOTLK Jewelcrafting",
        quests = { 12958, 12962, 12959, 12961, 12963, 12960 },
        max = 1,
        subtype = "wotlkprofessions"
    }
}
